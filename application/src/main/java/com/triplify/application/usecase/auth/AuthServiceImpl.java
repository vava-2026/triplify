...
public class AuthServiceImpl implements AuthService {
    ...
    @Override
    public Result<AuthResponse, ErrorResponse> login(LoginRequest command) {
        Result<LoginRequest, FieldError> validation = ValidationMapper.validate(command);
        if (validation.isFailure()) {
            log.debug("Login validation failed: {}", validation.getErrors());
            List<ErrorResponse> errors = validation.getErrors().stream().map(FieldError::toErrorResponse).toList();
        ...
        return userRepository.findByUsernameOrEmail(command.username(), command.username())
                ...
                .orElseGet(() -> {
                    ...
                    return Result.failure(ErrorResponse.of(ErrorCode.AUTH_INVALID_CREDENTIALS));
                });
    }

    private Result<AuthResponse, ErrorResponse> authenticate(User user, String rawPassword) {
